...
class Door(pygame.sprite.Sprite):
    ...
    def update_door(self, player):
        if (player.rect.x <= self.rect.x + 20 and player.rect.x >= self.rect.x - 20) and \
           (player.rect.y <= self.rect.y + 20 and player.rect.y >= self.rect.y - 20):
            return True
        else:
            return False
        ...
        if direction == "down":
            if (self.y2 != self.rect.y) and \
              (((p1.rect.x <= portal_opener.rect.x + 15 and p1.rect.x >= portal_opener.rect.x - 15) and \
                (p1.rect.y <= portal_opener.rect.y + 15 and p1.rect.y >= portal_opener.rect.y - 25)) or \
...
class Enemy(pygame.sprite.Sprite):
    ...
    def attack(self, player):
        if self.type == "Tank":
            if (player.rect.y <= self.rect.y + self.length and player.rect.y > self.rect.y - 25) and \
               ((player.rect.x >= self.rect.x) and (player.rect.x <= self.x + 160)):
                self.image = pygame.transform.scale(images['tank_enemy'], (self.width, self.length))
                self.rect.x += 0.75
            elif (player.rect.y <= self.rect.y + self.length and player.rect.y > self.rect.y - 25) and \
                 ((player.rect.x <= self.rect.x) and (player.rect.x >= self.x - 160)):
            ...
            if (player.rect.y <= self.rect.y + self.length and player.rect.y > self.rect.y - 25) and \
               ((player.rect.x >= self.rect.x) and (player.rect.x <= self.x + 200)):
                self.flipped_image = pygame.transform.flip(self.image, True, False)
...
class Player(pygame.sprite.Sprite):
    ...
    def jump(self):
        ...
        platform_hit_list = pygame.sprite.spritecollide(self, self.walls, False) + \
                            pygame.sprite.spritecollide(self, self.portals, False)
        self.rect.y -= 2
    ...
    for key in lake_list:
        if player.color != key:
            for lake in lake_list[key]:
                if (player.rect.x + 24 >= lake.rect.x and player.rect.x - 9 <= lake.rect.x) and \
                   (player.rect.y + 34 >= lake.rect.y and player.rect.y - 14 <= lake.rect.y):
                    player.life -= 1
            for enemy in enemies:
                if (player.rect.x + 25 >= enemy.rect.x and player.rect.x <= enemy.rect.x) and \
                   (player.rect.y + 25 >= enemy.rect.y and player.rect.y - 15 <= enemy.rect.y):
                    player.life -= 1

def high(player, list_of_trapeze):
    for trapeze in list_of_trapeze:
        if (player.rect.x >= trapeze.rect.x - 10 and player.rect.x <= trapeze.rect.x + 10) and \
           (player.rect.y <= trapeze.rect.y and player.rect.y > trapeze.rect.y - 200):
            return 1
        elif (player.rect.x >= trapeze.rect.x - 10 and player.rect.x <= trapeze.rect.x + 10) and \
             player.rect.y == trapeze.rect.y - 250:
            return 2

def score_total(player):
    if player.color == BLUE: 
        for coin in blue_coin_list:
            if (player.rect.x + 24 >= coin.x and player.rect.x - 9 <= coin.x) and \
               (player.rect.y + 24 >= coin.y and player.rect.y - 9 <= coin.y):
...
# Modified win function with a Next Level button
def win():
    global sc_map
    screen.fill(WHITE)
    font = pygame.font.Font('freesansbold.ttf', 82)
    ...
    next_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 75, SCREEN_HEIGHT // 2, 150, 50)
...
def return_to_menu():
    ...
    menu_text = return_font.render('Press the button R to restart the game!', True, WHITE, RED)
    menuRect = menu_text.get_rect()
    menuRect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 300)
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (x > 30 and x < 80) and (y > 20 and y < 70):
                ...
                details.main()
            # Check for Next Level button on win screen
            if sc_map == [2]:
    ...
    if sc_map == maps.level_three:
        ...
        if player1.color == (255, 0, 0):
            ...
            text = font.render('Color : Green', True, GREEN)
        textRect = text.get_rect()
        textRect.center = (SCREEN_WIDTH // 4 + 50, 40)
        screen.blit(text, textRect)
        font = pygame.font.Font('freesansbold.ttf', 30)
        if player1.color == (255, 0, 0):
            text = font.render('Color : Blue', True, BLUE)
        else:
            text = font.render('Color : Black', True, BLACK)
        textRect = text.get_rect()
        textRect.center = ((SCREEN_WIDTH // 2 + SCREEN_WIDTH) // 2, 40)
        screen.blit(text, textRect)
        score(final_score)
